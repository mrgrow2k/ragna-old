...
//
// Bootup the masternode, look for a 10000 RAGNA input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in ragnarok.conf */
...
{
    CragnarokcoinAddress address(collateralAddress);
