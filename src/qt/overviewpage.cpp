...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeragnarokAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeragnarokAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeragnarokAmount = strAnonymizeragnarokAmount.remove(strAnonymizeragnarokAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeragnarokAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeragnarokAmount*COIN) nMaxToAnonymize = nAnonymizeragnarokAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeragnarokAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeragnarokAmount));
        strAnonymizeragnarokAmount = strAnonymizeragnarokAmount.remove(strAnonymizeragnarokAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeragnarokAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
