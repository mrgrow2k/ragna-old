    ...
    model(0)
{
    ...
    ui->RAGNAAvailableLabel->setTextFormat(Qt::RichText);
    ...
    ui->BittrexRAGNALabel->setTextFormat(Qt::RichText);
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "RAGNA(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)" << "RAGNA(SIZE)" << "TOTAL(BTC)" << "SUM(BTC)");
    /*OrderBook Table Init*/

    /*Market History Table Init*/
    ui->MarketHistoryTable->setColumnCount(5);
    ui->MarketHistoryTable->verticalHeader()->setVisible(false);
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(RAGNA)"<<"TOTAL COST(BTC");
...
void tradingDialog::UpdaterFunction(){
    //RAGNAst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://bittrex.com/api/v1.1/public/GetMarketSummary?market=btc-RAGNA");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://bittrex.com/api/v1.1/public/getorderbook?market=BTC-RAGNA&type=both&depth=50");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://bittrex.com/api/v1.1/public/getmarkethistory?market=BTC-RAGNA&count=100");
      return Response;
}
...
QString tradingDialog::BuyRAGNA(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://bittrex.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-RAGNA&quantity=";
...
QString tradingDialog::SellRAGNA(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://bittrex.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-RAGNA";
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getdepositaddress?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=RAGNA";
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-RAGNA&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>RAGNA Volume:</b> <span style='font-weight:bold; font-size:12px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> RAGNA");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    double RAGNASupply = 0;
    double RAGNADemand = 0;
    double BtcSupply = 0;
    double BtcDemand = 0;
    ...
    ui->RAGNASupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(RAGNASupply,'i',8) + "</span><b> RAGNA</b>");
    ...
    ui->RAGNADemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(RAGNADemand,'i',8) + "</span><b> RAGNA</b>");
...
void tradingDialog::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
                case 0:    //buy tab is active
                    ...
                    Response2 = GetBalance("RAGNA");
                    Response3 = GetOrderBook();

                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BtcAvailableLabel, *ui->RAGNAAvailableLabel, Response, Response2);
                    }
                ...
                case 1: //Cross send tab active
                    Response = GetBalance("RAGNA");
                    Response2 = GetBalance("BTC");
                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BittrexRAGNALabel, *ui->BittrexBTCLabel, Response, Response2);
                    }
                ...
                case 5://show balance tab
                    ...
                    Response = GetBalance("RAGNA");
                    if(Response.size() > 0 && Response != "Error"){
                        DisplayBalance(*ui->RAGNABalanceLabel,*ui->RAGNAAvailableLabel_2,*ui->RAGNAPendingLabel, QString::fromUtf8("RAGNA"),Response);
                    }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputRAGNA->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    QString balance = GetBalance("RAGNA");
    ...
    double AvailableRAGNA = BalanceObject["Available"].toDouble();
...
{
    //calculate amount of BTC that can be gained from selling RAGNA available balance
    QString responseA = GetBalance("RAGNA");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONObject(responseA);

    double AvailableRAGNA = ResultObject["Available"].toDouble();

    ui->UnitsInputRAGNA->setText(str.number(AvailableRAGNA,'i',8));
}
...
{
    double Quantity = ui->BittrexRAGNALabel->text().toDouble();
    double Received = 0;
    double Qty = 0;
    double Price = 0;
    QString buyorders = GetOrderBook();
...
void tradingDialog::on_Withdraw_Max_Amount_clicked()
{
    ...
    ui->WithdrawUnitsInput->setText(str.number(AvailableRAGNA,'i',8));
}
...
void tradingDialog::on_BuyRAGNA_clicked()
{
    double Rate;
    double Quantity;
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  BuyRAGNA(Order,Quantity,Rate);
...
void tradingDialog::on_SellRAGNABTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputRAGNA->text().toDouble();
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  SellRAGNA(Order,Quantity,Rate);
        QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
        QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
    ...
    if(!ctx.isValid())
    {
        ...
        foreach (const QJsonValue & value, BuyArray)
        {
            ...
            if ( ((Quantity / x) - y) > 0 )
            {
                ...
                QString SellResponse = SellRAGNA(Order,y,x);
                ...
                QString SellResponse = SellRAGNA(Order,(Quantity / x),x);
                QJsonDocument SelljsonResponse = QJsonDocument::fromJson(SellResponse.toUtf8());          //get json from str.
                QJsonObject SellResponseObject = SelljsonResponse.object();                              //get json obj
                ...
                if (SellResponseObject["success"].toBool() == false){
                    ...
                    if (ResponseObject["success"].toBool() == false){
                        ...
                        if (ResponseObject["success"].toBool() == false){
                            ...
                            QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" RAGNA for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                        }
                    } else if (ResponseObject["success"].toBool() == true){
                        QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" RAGNA for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                    }
...
void tradingDialog::on_UnitsInputRAGNA_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}
