...
CScript _createmultisig(const Array& params)
{
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        CragnarokcoinAddress address(ks);
...
Value createmultisig(const Array& params, bool fHelp)
{
    ...
    CragnarokcoinAddress address(innerID);
...
Value getnewaddress(const Array& params, bool fHelp)
{
    ...
    return CragnarokcoinAddress(keyID).ToString();
}


CragnarokcoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CragnarokcoinAddress(account.vchPubKey.GetID());
}
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    CragnarokcoinAddress address(params[0].get_str());
    if (!address.IsValid())
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CragnarokcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CragnarokcoinAddress& address = item.first;
    ...
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
           ...
           "1. \"ragnarokaddress\"  (string, required) The Ragnarok address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in RAGNA to send. eg 0.1\n"
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(CragnarokcoinAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CragnarokcoinAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CragnarokcoinAddress(address).Get())->second);
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    ...
    CragnarokcoinAddress addr(strAddress);
    if (!addr.IsValid())
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nResult:\n"
            "amount   (numeric) The total amount in RAGNA received at this address.\n"
            "\nExamples:\n"
    ...
    // Bitcoin address
    CragnarokcoinAddress address = CragnarokcoinAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Ragnarok address");
...
Value getreceivedbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "2. minconf          (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount              (numeric) The total amount in RAGNA received for this account.\n"
            "\nExamples:\n"
...
Value movecmd(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            ...
            "\nExamples:\n"
            "\nMove 0.01 RAGNA from the default account to the account named tabby\n"
            + HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 RAGNA timotei to akiko with a comment and funds have 10 confirmations\n"
            + HelpExampleCli("move", "\"timotei\" \"akiko\" 0.01 10 \"happy birthday!\"") +
            "\nAs a json RAGNA call\n"
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            ...
            "3. amount                (numeric, required) The amount in RAGNA. (transaction fee is added on top).\n"
            ...
            "\"transactionid\"        (string) The transaction id.\n"
            "\nExamples:\n"
            "\nSend 0.01 RAGNA from the default account to the address, must have at least 1 confirmation\n"
    ...
    string strAccount = AccountFromValue(params[0]);
    CragnarokcoinAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Ragnarok address");
    CAmount nAmount = AmountFromValue(params[2]);
...
Value sendmany(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The Ragnarok address is the key, the numeric amount in RAGNA is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    set<CragnarokcoinAddress> setAddress;
    vector<pair<CScript, int64_t> > vecSend;

    int64_t totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CragnarokcoinAddress address(s.name_);
        if (!address.IsValid())
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    ...
    return CragnarokcoinAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CragnarokcoinAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CragnarokcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CragnarokcoinAddress, tallyitem>::iterator it = mapTally.find(address);
...
Value listreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in RAGNA received by the address\n"
...
Value listtransactions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"amount\": x.xxx,          (numeric) The amount in RAGNA. This is negative for the 'send' category, and for the\n"
...
Value listsinceblock(const Array& params, bool fHelp)
{
    if (fHelp)
       throw runtime_error(
            ...
            "    \"amount\": x.xxx,          (numeric) The amount in RAGNA. This is negative for the 'send' category, and for the 'move' category for moves \n"
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending RAGNA\n"
            "\nArguments:\n"
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending Ragnarok\n"
...
Value settxfee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in RAGNA/kB rounded to the nearest 0.00000001\n"
            "\nResult\n"
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new Ragnarok stealth address for receiving payments anonymously.  ");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid Ragnarok stealth address."));
        return result;
    };
