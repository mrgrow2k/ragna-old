...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <ragnarokaddress> <amount>\n"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  stop         - Stop masternode configured in ragnarok.conf\n"
                ...
                "  vote         - Vote on a Ragnarok initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CragnarokcoinSecret(secret).ToString();
    }
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "pubkey" && strMode != "rank" && strMode != "status" && strMode != "addr" && strMode != "votes" && strMode != "lastpaid"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a Ragnarok initiative (can be additionally filtered, partial match)\n"
